...
# Page Config
st.set_page_config(page_title="PVF: Unbreakable Engine Demo", layout="wide")
...
# Style Preset
style_preset = st.sidebar.radio("Style Preset", ("Dopamine_Spike", "Cinematic_Stoic"))
...
script_input = st.sidebar.text_area(
    ...
    height=150,
    help="Enter the text script for the video generation. Each sentence will become a video segment.",
)
...
failure_mode_label = st.sidebar.radio(
    ...
    help="Select which failure scenario to simulate. 'Bhuchal Mode' forces the system to its lowest resilience layer.",
)
...
if st.button("Generate Blueprint & Simulate Render", type="primary"):
    ...
    if failure_mode == 0:
        ...
        st.warning(
            "Simulation Complete: System fell back to Procedural Assets due to missing local files."
        )
    else:
