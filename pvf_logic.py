...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Usually one track for the whole project
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            # Text segment is passed as description/content
            audio_data = self.asset_manager.get_asset(text[:20], failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    if g_audio:
        ...
        logs.append(f"  > Source: {g_audio.get('path', g_audio.get('type', 'Unknown'))}")
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {asset['type']} (Color: {asset.get('color', '#000')})"

        logs.append(f"  > Action: {action}")

        if audio:
            logs.append(f"  > Voiceover: {audio.get('description', audio.get('path', 'Unknown'))}")
            logs.append(f"    - Layer: {audio['layer_used']}")
