...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            filename_suffix = ".mp4"
            if asset_type == "music": filename_suffix = ".mp3"
            if asset_type == "voiceover": filename_suffix = ".mp3"

            asset_filename = f"{asset_description.replace(' ', '_').upper()}{filename_suffix}"
        ...
        if asset_type == "video":
            return {
                ...
                "desc": "Animated binary text overlay",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            if asset_data["source"] == "procedural" and asset_data["type"] == "video_generator":
            ...
            # Get Voiceover (Audio)
            # Use the text content as the description
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('path', global_audio.get('details'))}")
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            details = asset.get('details', 'Unknown')
            color = asset.get('color', 'N/A')
            action = f"Rendering {segment['duration']}s of Procedural {asset['type']} ({details}) (Color: {color})"

        logs.append(f"  > Action: {action}")

        # Voiceover Log
        if vo:
            logs.append(f"  > [AUDIO] Voiceover: {vo['type']}")
