...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            prefix = "V" if asset_type == "video" else "M" if asset_type == "music" else "S"
            asset_filename = f"{prefix}_{asset_description.replace(' ', '_').upper()[:10]}.mp4"
        ...
        # --- LAYER 3: PROCEDURAL BACKUP (Bhuchal Mode) ---
        return self._get_procedural_fallback(asset_type, asset_description)

    def _get_procedural_fallback(self, asset_type, description):
        """
        Implements the 'Never Nothing' procedural generation logic.
        """
        if asset_type == "video":
            return {
                "source": "procedural",
                "type": "CRT_INTERFACE",
                "description": "Procedural Glitch Screen with animated binary text overlay",
                "params": {
                    "effect": "chromatic_aberration",
                    "scan_lines": True,
                    "overlay_text": f"SYSTEM_ERR: {description}"
                },
            ...
            return {
                ...
                "description": "Tension Soundscape: Low-frequency Sine Wave Drone",
                "params": {
                    "frequency": "low",
                    "waveform": "sine",
            ...
            return {
                "source": "procedural",
                "type": "robotic_speech",
                "description": "Robotic Speech / Subtitled Text Only",
                "params": {
                    "engine": "basic_tts_fallback",
                    "voice": "robot_v1"
                },
                "layer_used": "Layer 3 (Procedural Backup)"
            }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Assets (Music) - Request once for the whole video
        music_desc = f"Background music for {style_preset_name}"
        ...
        for idx, text in enumerate(segments):
            ...
            if visual_asset["source"] == "procedural":
                if style_preset_name == "Dopamine_Spike":
                    visual_asset["params"]["color_override"] = "#FF00FF" # Neon Pink for fallback in dopamine
                else:
                    visual_asset["params"]["color_override"] = "#222222" # Dark Grey for fallback in stoic
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {music.get('type', 'Unknown')} ({music.get('source', 'Unknown')})")
    if music.get('source') == 'procedural':
        logs.append(f"  > Music Fallback: {music.get('description')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Visual Log
        logs.append(f"  > [Visual] Requesting: '{v_asset.get('path', v_asset.get('type'))}'")
        ...
        if v_asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {v_asset['type']} (Desc: {v_asset.get('description', '')})"
        logs.append(f"    - Action: {action}")

        # Audio Log
        logs.append(f"  > [Audio] Requesting Voiceover")
        logs.append(f"    - Layer Used: {a_asset['layer_used']}")
        if a_asset["source"] == "procedural":
             logs.append(f"    - Action: Generating {a_asset['type']} ({a_asset.get('description')})")
        else:
             logs.append(f"    - Action: Playing Local Audio {a_asset.get('path')}")
