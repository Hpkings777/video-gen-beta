...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            suffix = suffix_map.get(asset_type, ".dat")

            asset_filename = f"{asset_description.replace(' ', '_').upper()}{suffix}"
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            voiceover_asset = self.asset_manager.get_asset(asset_desc, failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    if g_audio:
        logs.append(f"Global Audio Track: {g_audio.get('desc', g_audio.get('path', 'Unknown'))}")
        logs.append(f"  > Layer Used: {g_audio['layer_used']}")
    ...
    for segment in blueprint["timeline"]:
        ...
        if v_asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local Video: {v_asset['path']}"
        else:
            action = f"Rendering {segment['duration']}s of Procedural {v_asset['type']} (Color: {v_asset['color']})"
        logs.append(f"    Action: {action}")

        # Audio Log
        if a_asset:
             logs.append(f"  > [AUDIO] Requesting Voiceover: '{a_asset.get('desc', a_asset.get('path', 'PROCEDURAL'))}'")
