    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "music":
            ...
            return {
                ...
                "type": "tts_basic",
                "desc": "Robotic Speech",
                "text_content": asset_description, # The text to speak
                "layer_used": layer_name
            }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            voice_asset = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('desc', global_audio.get('path'))}")
        if global_audio.get('freq'):
             logs.append(f"  > ({global_audio['freq']}Hz Sine)")
        logs.append(f"  > Layer Used: {global_audio['layer_used']}")
        logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        asset = segment["visual_asset"]
        voice_asset = segment.get("audio_asset")
        ...
        # Voiceover Log
        if voice_asset:
             logs.append(f"  > [AUDIO] Voiceover: {voice_asset.get('desc', voice_asset.get('path'))}")
             logs.append(f"  > Layer Used: {voice_asset['layer_used']}")
