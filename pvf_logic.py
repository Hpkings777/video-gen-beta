...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        # Generates resilient procedural assets based on type
        result = {
            "source": "procedural",
        ...
        if asset_type == "video":
            result["type"] = "backup_generator::CRT_INTERFACE"
            result["details"] = "Animated binary text overlay"
            result["color"] = "#000000" # Default base
            result["text"] = "SYSTEM FALLBACK: " + asset_description
        elif asset_type == "music":
            result["type"] = "backup_generator::SINE_WAVE"
            result["details"] = "Low-freq sine wave"
        elif asset_type == "voiceover":
            result["type"] = "backup_generator::ROBOTIC_SPEECH"
            result["details"] = "Text-to-Speech fallback"
        else:
            result["type"] = "backup_generator::UNKNOWN"
            result["details"] = "Generic Fallback"
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            voice_asset = self.asset_manager.get_asset(text[:20], failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio['desc']}")
        if audio["source"] == "local":
             logs.append(f"  > Source: {audio['path']}")
        else:
             logs.append(f"  > Type: {audio['type']}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Visual Log
        logs.append(f"  > [VISUAL] Requesting Asset: '{asset.get('path', asset.get('type'))}'")
        ...
        if asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {asset['type']}"
            if "details" in asset:
                 action += f" ({asset['details']})"
            if "color" in asset:
        ...
        # Voiceover Log
        if voice:
            logs.append(f"  > [AUDIO] Voiceover: {voice['desc']}...")
            if voice["source"] == "local":
                 logs.append(f"    > Source: {voice['path']}")
            else:
                 logs.append(f"    > Type: {voice['type']}")
                 logs.append(f"    > Details: {voice['details']}")
            logs.append(f"    > Layer Used: {voice['layer_used']}")
