...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('desc', global_audio.get('path'))} ({global_audio.get('type')})")
    ...
    for segment in blueprint["timeline"]:
        ...
        if audio:
             logs.append(f"  > [AUDIO] Requesting Voiceover: '{audio.get('path', 'PROCEDURAL')}'")
             logs.append(f"    - Layer Used: {audio['layer_used']}")
             if audio["source"] == "local":
                 logs.append(f"    - Action: Playing Local Audio: {audio['path']}")
             else:
                 logs.append(f"    - Action: Generating Procedural {audio['type']} ({audio['desc']})")
