...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            # Determine extension and prefix based on type
            if asset_type == "video":
        ...
        # Specific fallbacks per asset type
        if asset_type == "video":
            return {
                "source": "procedural",
                "type": "solid_color",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Music Track
        music_desc = f"{style_preset_name}"
...
def renderer_stub(blueprint):
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            if asset['type'] == 'solid_color':
                 # Bhuchal mode check
                 if asset.get('text', '').startswith("SYSTEM FALLBACK"):
                      logs.append("  > [VISUAL] Procedural Glitch Screen Applied")

        logs.append(f"  > Action: {action}")

        # Log Voiceover
        if vo_asset:
             logs.append(f"  > [AUDIO] Voiceover: {vo_asset.get('desc', 'VO')}")
