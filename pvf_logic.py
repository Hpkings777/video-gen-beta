...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            asset_filename = f"{prefix}_{asset_description.replace(' ', '_').upper()}.{ext}"
        ...
        if asset_type == "video":
             return {
                "source": "procedural",
                "type": "video",
                "variant": "backup_generator::CRT_INTERFACE",
                "color": "#000000", # Default black, can be overridden
             ...
             return {
                "source": "procedural",
                "type": "music",
                "variant": "Tension Soundscape",
                "details": "150Hz Sine Wave Drone",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "voiceover",
                "variant": "Robotic Speech",
                "details": "Basic TTS / Subtitled Text Only",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
...
def renderer_stub(blueprint):
    ...
    desc = audio.get('variant') if audio.get('source') == 'procedural' else audio.get('path', 'Unknown')
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > Requesting Video: '{asset.get('path') if asset.get('source') == 'local' else asset.get('variant')}'")
        ...
        # Audio Log
        logs.append(f"  > [AUDIO] Voiceover: {vo.get('variant') if vo.get('source') == 'procedural' else vo.get('path', 'Unknown')}")
        logs.append(f"  > Layer Used: {vo.get('layer_used', 'Unknown')}")
