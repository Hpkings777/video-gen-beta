...
class AssetManager:
    ...
    def get_asset(self, asset_description, asset_type="video", failure_mode=0):
        """
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # 1. Get Global Audio Track
        bg_music_desc = f"Background_Track_{style_preset_name}"
        global_audio = self.asset_manager.get_asset(bg_music_desc, "music", failure_mode)
        ...
        for idx, text in enumerate(segments):
            ...
            # Layer 2/3 Resolution for Visuals
            visual_asset = self.asset_manager.get_asset(asset_desc, "video", failure_mode)

            # Layer 2/3 Resolution for Voiceover
            # Use the text itself as the description for TTS
            voiceover_asset = self.asset_manager.get_asset(text, "voiceover", failure_mode)
...
def renderer_stub(blueprint):
    ...
    if audio:
        ...
        if audio["source"] == "procedural":
             logs.append(f"  > Layer Used: {audio['layer_used']} ({audio.get('freq')}Hz Sine)")
        else:
    ...
    for segment in blueprint["timeline"]:
        ...
        if vo:
             logs.append(f"  > [AUDIO] Voiceover: {vo.get('desc', 'Text Segment')}")
