...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "video":
            ...
            fallback_data.update({
                "type": "ROBOTIC_SPEECH",
                "description": "Local Basic TTS / Subtitles Only"
            })
        else:
            # Generic fallback
            fallback_data.update({
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            voiceover_asset = self.asset_manager.get_asset(f"VO_Segment_{idx}", failure_mode, asset_type="voiceover")

            # Apply Style specific procedural overrides if needed
            if visual_asset["source"] == "procedural" and "color" in visual_asset:
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {audio.get('path', audio.get('type', 'Unknown'))}")
    logs.append(f"  > Layer Used: {audio.get('layer_used', 'N/A')}")
    if audio.get("source") == "procedural":
        logs.append(f"  > Audio Fallback: {audio.get('description', 'Standard Fallback')}")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        v_asset = segment["visual_asset"]
        vo_asset = segment.get("voiceover_asset", {})

        logs.append(f"[Segment {segment['id']}] Duration: {segment['duration']}s")

        # Visuals
        logs.append(f"  > [Visual] Requesting Asset: '{v_asset.get('path', v_asset.get('type', 'PROCEDURAL'))}'")
        logs.append(f"  > [Visual] Layer Used: {v_asset['layer_used']}")
        ...
        # Voiceover
        logs.append(f"  > [Voiceover] Layer Used: {vo_asset.get('layer_used', 'N/A')}")
        if vo_asset.get("source") == "procedural":
             logs.append(f"  > [Voiceover] Fallback: {vo_asset.get('description', 'TTS')}")
