...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio.get('name', 'Unknown')}")
        logs.append(f"  > Source: {audio['source']}")
        logs.append(f"  > Layer Used: {audio['layer_used']}")
        if audio["source"] == "procedural":
             logs.append(f"  > Action: Generating {audio['frequency']}Hz {audio['type']} Drone")
        else:
             logs.append(f"  > Action: Mixing Local Audio File: {audio['path']}")
