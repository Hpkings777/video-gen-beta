    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            if asset_type == "video":
                ...
                asset_filename = f"Music_Track_{asset_description}.{ext}"
            elif asset_type == "voiceover":
                 asset_filename = f"VO_{asset_description.replace(' ', '_')}.{ext}"
            else:
                 asset_filename = f"Asset_{asset_description}.{ext}"
        ...
        if asset_type == "video":
            return {
                ...
                "color": "#000000", # Default, can be overridden by style
            ...
            return {
                ...
                "details": "Code-generated, low-frequency Sine Wave Drone",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
            ...
            return {
                "source": "procedural",
                "type": "Robotic Speech",
                "details": "Local, self-hosted basic TTS",
...
class JSONBuilder:
    ...
    STYLE_PRESETS = {
        "Dopamine_Spike": {
            ...
            "transition": "cut_fast",
            "video_generator": "backup_generator::CRT_INTERFACE"
        },
        "Cinematic_Stoic": {
            "min_duration": 4.0,
            "max_duration": 8.0,
            ...
            "video_generator": "backup_generator::MINIMALIST_VOID"
        }
    }
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # 1. Global Assets (Music)
        global_music = self.asset_manager.get_asset(
            style_preset_name,
            failure_mode,
            asset_type="music"
        )
        ...
        for idx, text in enumerate(segments):
            ...
            # 3. Audio Asset (Voiceover)
            # Use first 20 chars of text as description key
            vo_desc = text[:20]
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {music.get('desc', music.get('type', 'Unknown'))}")
    if music.get("source") == "procedural":
        logs.append(f"  > Details: {music.get('details')}")
    else:
        logs.append(f"  > Source: {music.get('path')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Visual Log
        logs.append(f"  > Requesting Visual: '{v_asset.get('path', 'PROCEDURAL')}'")
        ...
        if v_asset["source"] == "local":
            ...
            # Procedural
            action = f"Rendering {segment['duration']}s of Procedural {v_asset.get('type')} (Color: {v_asset.get('color')})"

        logs.append(f"  > Action: {action}")

        # Audio Log
        logs.append(f"  > [AUDIO] Voiceover: {a_asset.get('type', 'Unknown')}")
        if a_asset.get("source") == "procedural":
             logs.append(f"    > Details: {a_asset.get('details')}")
        else:
             logs.append(f"    > Source: {a_asset.get('path')}")
