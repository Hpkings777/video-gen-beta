...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio.get('name', audio.get('path', 'Unknown'))}")
        logs.append(f"  > Layer Used: {audio['layer_used']}")
        if audio["source"] == "procedural":
             logs.append(f"  > Action: Generating {audio.get('freq')} {audio.get('type')} Drone")
        else:
             logs.append(f"  > Action: Mixing Local Track {audio.get('path')}")
