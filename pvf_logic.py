...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "music":
            # Layer 3 Music: Tension Soundscape
            return {
                "source": "procedural",
            ...
            # Layer 3 Video: Procedural Glitch/Solid Color
            return {
                "source": "procedural",
                "type": "solid_color",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Layer 3 fallback: Replaces missing music with tension sine wave to prevent silent render.
        global_audio = self.asset_manager.get_asset(
            f"Background_Track_{style_preset_name}", failure_mode, asset_type="music"
        )
...
def renderer_stub(blueprint):
    ...
    # Log Global Audio Info
    audio = blueprint.get("global_audio", {})
    audio_name = (
        audio.get("name", "Unknown")
        if audio.get("source") == "procedural"
        else audio.get("path", "Unknown")
    )
