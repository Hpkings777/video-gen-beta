...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            asset_data = self.asset_manager.get_asset(
                asset_desc, failure_mode, asset_type="video"
            )

            # Get Voiceover
            voice_data = self.asset_manager.get_asset(
                text, failure_mode, asset_type="voiceover"
            )
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {bg_music.get('description', 'Unknown')}")
    logs.append(f"  > Source: {bg_music.get('source', 'Unknown')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Log Voiceover
        if voice.get("source") == "local":
            vo_action = f"Voiceover: Local File {voice.get('path')}"
        elif voice.get("source") == "procedural":
            vo_action = f"Voiceover: Robotic TTS"
        else:
            vo_action = "Voiceover: None"

        logs.append(f"  > {vo_action}")
