    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            if asset_type == "video":
                ...
                asset_filename = f"{asset_description.replace(' ', '_').upper()}.dat"
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Get Global Audio
        # Use style preset name as the description for music
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover
            # Use text content (truncated) as description
            vo_desc = text[:20]
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('desc')}")
        if global_audio.get("source") == "procedural":
             logs.append(f"  > Details: {global_audio.get('details')}")
        elif global_audio.get("source") == "local":
             logs.append(f"  > Source: {global_audio.get('path')}")
        logs.append(f"  > Layer Used: {global_audio.get('layer_used')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            if asset.get("type") == "video_generator":
                 action = f"Rendering {segment['duration']}s of Procedural {asset.get('desc')} (Color: {asset.get('color')})"
            else:
                 action = f"Rendering {segment['duration']}s of Procedural {asset.get('type')} (Color: {asset.get('color')})"

        logs.append(f"  > Action: {action}")

        if audio_asset:
             logs.append(f"  > [AUDIO] Voiceover: {audio_asset.get('desc')}")
