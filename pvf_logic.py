...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type='video'):
        ...
        if failure_mode == 0:
            # Real check for local file existence
            # Naming convention based on asset type
            if asset_type == 'video':
                filename = f"{asset_description.replace(' ', '_').upper()}.mp4"
            elif asset_type == 'music':
                filename = f"Music_Track_{asset_description.replace(' ', '_')}.mp3"
            elif asset_type == 'voiceover':
                filename = f"VO_{asset_description.replace(' ', '_')}.mp3"
            else:
                filename = f"{asset_description}.dat"

            file_path = os.path.join(self.asset_dir, filename)

            # NOTE: For the "Normal Test" (Mode 0), if the file doesn't exist, we technically fail Layer 2.
            # However, the user request/demo instructions often imply "Simulate Success".
            # But the final_goal.md says "Local asset_id found" for Layer 2.
            # To adhere to "Measure First", we should check existence.
            # If it's missing, we fall back to Layer 3 (and maybe log a warning if we expected success).
            ...
            if os.path.exists(file_path):
                return {
                    ...
                    "path": file_path,
                    "type": asset_type,
                    "desc": asset_description, # Useful for logging
                    "layer_used": "Layer 2 (Local Assets)"
                }
            else:
                # If we are in Mode 0 but file is missing, we must fall back to Layer 3.
                # This respects reality over simulation.
                logger.warning(f"Layer 2 Failed: File {filename} not found in {self.asset_dir}.")
                pass

        elif failure_mode == 1:
            # Simulate Layer 2 Failure (File not found)
            logger.warning("Layer 2 Failed: File not found (Simulated Failure)")
        ...
        if asset_type == 'video':
            return {
                ...
                "color": "#000000", # Default, overridden by style usually
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Using style preset name as description for music track
...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio.get('desc', 'Unknown') if audio['source'] == 'local' else audio['type']}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > Requesting Video: '{asset.get('path', 'PROCEDURAL')}'")
        ...
        # Voiceover Log
        if vo:
             logs.append(f"  > [AUDIO] Voiceover: {vo.get('desc', vo['type'])}")
