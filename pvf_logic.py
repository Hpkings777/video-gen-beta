...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        global_audio = self.asset_manager.get_asset(
            f"Background_Track_{style_preset_name}",
            failure_mode,
            asset_type="music"
        )
        ...
        for idx, text in enumerate(segments):
            ...
            video_asset_data = self.asset_manager.get_asset(asset_desc, failure_mode, asset_type="video")

            # Get Voiceover Asset
            audio_asset_data = self.asset_manager.get_asset(text[:20], failure_mode, asset_type="voiceover")

            # Apply Style specific procedural overrides if needed
            if video_asset_data["source"] == "procedural":
                if style_preset_name == "Dopamine_Spike":
                    video_asset_data["color"] = "#FF00FF" # Neon Pink for fallback in dopamine
                else:
                    video_asset_data["color"] = "#222222" # Dark Grey for fallback in stoic

            segment_data = {
                "id": f"seg_{idx:03d}",
                "text_content": text,
                "duration": round(duration, 2),
                "visual_asset": video_asset_data,
                "audio_asset": audio_asset_data,
                "style_config": {
                    "font": style["font"],
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {global_audio.get('desc', 'Unknown')} ({global_audio.get('type', 'Unknown')})")
    logs.append(f"  > Layer Used: {global_audio.get('layer_used', 'Unknown')}")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        visual = segment["visual_asset"]
        audio = segment["audio_asset"]

        logs.append(f"[Segment {segment['id']}] Duration: {segment['duration']}s")

        # Visual Log
        logs.append(f"  > [VIDEO] Requesting Asset: '{visual.get('path', 'PROCEDURAL')}'")
        ...
        # Audio Log
        logs.append(f"  > [AUDIO] Voiceover: {audio.get('desc', audio.get('path', 'Unknown'))}")
