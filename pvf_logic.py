...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            # Filename Logic:
            # Video: COFFEE.mp4 (Upper)
            # Music: Music_Track_Dopamine_Spike.mp3 (Preserve Case)
            # VO: VO_Coffee_is_life.mp3 (Preserve Case)
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Uses style name as description
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            # Use full text (truncated for desc)
            vo_desc = text[:20]
...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio.get('type')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        vo = segment.get("voiceover_asset")
        ...
        if asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {asset.get('type')} (Color: {color})"

        logs.append(f"  > Action: {action}")

        # Voiceover Log
        if vo:
            logs.append(f"  > [AUDIO] Voiceover: {vo.get('type')}")
