...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        return {"source": "error", "type": "unknown", "layer_used": "Layer 3 (Failed)"}
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Audio Track
        bg_track_name = f"Background_Track_{style_preset_name}"
        global_audio = self.asset_manager.get_asset(bg_track_name, failure_mode, asset_type="music")
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Asset (Voiceover)
            voice_data = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    # Log Global Audio
    bg_audio = blueprint.get("global_audio", {})
    logs.append(f"Global Audio Track: {bg_audio.get('description', bg_audio.get('path', 'Unknown'))}")
    logs.append(f"  > Source: {bg_audio.get('source')} | Layer: {bg_audio.get('layer_used')}")
    logs.append("-" * 40)
    ...
    for segment in blueprint["timeline"]:
        ...
        logs.append(f"  > [Visual] Layer Used: {asset['layer_used']}")
        ...
        # Audio Log
        logs.append(f"  > [Audio] Voiceover: {audio.get('voice', 'Local File')}")
        logs.append(f"  > [Audio] Layer Used: {audio['layer_used']}")
