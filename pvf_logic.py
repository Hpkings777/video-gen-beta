...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            asset_filename = f"{asset_description.replace(' ', '_').upper() if asset_type == 'video' else asset_description}.{ext}"
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            if asset_data["source"] == "procedural" and asset_data.get("type") == "backup_generator::CRT_INTERFACE":
...
def renderer_stub(blueprint):
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            details = asset.get('details', 'Generic')
        ...
        if vo.get("source") == "procedural":
             logs.append(f"  > Details: {vo.get('details', 'None')}")
