    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            if asset_type == "video":
                ...
                desc_key = f"asset_id::{clean_desc[:5].upper()}_00X"
            elif asset_type == "music":
                asset_filename = f"Music_Track_{clean_desc}.mp3"
                desc_key = f"music_id::M_{clean_desc[:3].upper()}"
            elif asset_type == "voiceover":
                asset_filename = f"VO_{clean_desc}.mp3"
                desc_key = f"tts_ref::S_{clean_desc[:3].upper()}"
            else:
                asset_filename = f"Asset_{clean_desc}.dat"
                desc_key = "unknown_id"
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            # Use the text content as the description/source for the voiceover
...
def renderer_stub(blueprint):
    ...
    if audio:
        ...
        if audio["source"] == "local":
             ...
             logs.append(f"  > Details: {audio['type']}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Audio Log
        if audio:
             logs.append(f"  > [AUDIO] Voiceover: {audio.get('desc', 'Unknown')}")
             if audio["source"] == "local":
                  logs.append(f"  > Source: {audio['path']}")
             else:
                  logs.append(f"  > Details: {audio['type']}")
