    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "video":
            return {
                "source": "procedural",
                "type": "video",
                "subtype": "glitch_screen",
                "color": "#000000", # Default black, overridden by builder
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # 1. Global Audio (Music)
        # Using style preset name as a key for the music track
        ...
        for idx, text in enumerate(segments):
            ...
            if video_asset["source"] == "procedural" and video_asset["type"] == "video":
            ...
            # Get Voiceover Asset
            # Using the segment ID and text snippet as description
            vo_desc = f"VO_{idx:03d}"
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {bg_music.get('desc', bg_music.get('path', 'Unknown'))}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > [VIDEO] Requesting Asset: '{video_asset.get('path', 'PROCEDURAL')}'")
        logs.append(f"    Layer Used: {video_asset.get('layer_used', 'Unknown')}")
        if video_asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local File: {video_asset['path']}"
        else:
        ...
        # Audio/VO Log
        logs.append(f"  > [AUDIO] Requesting Voiceover: '{audio_asset.get('path', 'PROCEDURAL')}'")
        logs.append(f"    Layer Used: {audio_asset.get('layer_used', 'Unknown')}")
        if audio_asset.get("source") == "local":
             audio_action = f"Mixing Local VO File: {audio_asset['path']}"
        else:
             audio_action = f"Synthesizing Procedural {audio_asset.get('subtype', 'Voice')}"
        logs.append(f"    Action: {audio_action}")
