    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "video":
            ...
            return {
                ...
                "params": {"frequency": "low", "pattern": "tension"},
                "layer_used": "Layer 3 (Procedural Backup)"
            }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Audio (Music) - Added as per Resilience Matrix
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('path', global_audio.get('type'))}")
    ...
    for segment in blueprint["timeline"]:
        ...
        logs.append(f"  > Video Source: '{asset.get('path', 'PROCEDURAL')}'")
        logs.append(f"    - Layer Used: {asset['layer_used']}")

        if vo_asset:
             logs.append(f"  > Voiceover Source: '{vo_asset.get('path', vo_asset.get('type'))}'")
        ...
        if asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {asset['type']} (Color: {color})"
