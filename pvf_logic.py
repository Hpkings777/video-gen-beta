...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            asset_filename = f"{asset_type.upper()}_{asset_description.replace(' ', '_').upper()}{ext}"
            if asset_type == "video": # Keep original naming for video to match legacy
                asset_filename = f"{asset_description.replace(' ', '_').upper()}{ext}"

             # Voiceover specific naming
        ...
        # Generates specific procedural data based on asset type
        ...
        if asset_type == "video":
            fallback_data.update({
                ...
                "overlay": "Animated binary text overlay",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Music Track
        # For simplicity, using style name as music description
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Asset (Voiceover)
            # Use text content snippet for ID
            ...
            # Only for visual procedural assets (Video)
...
def renderer_stub(blueprint):
    ...
    if bg_music.get("source") == "procedural":
        ...
        logs.append(f"  > File: {bg_music.get('path', 'Unknown')}")
    logs.append("-" * 40)
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            # Check if color exists (it might not for non-video procedural types, but this is visual_asset)
            color_info = f"(Color: {asset.get('color', 'N/A')})"
            action = f"Rendering {segment['duration']}s of Procedural {asset.get('type', 'Unknown')} {asset.get('details', '')} {color_info}"
