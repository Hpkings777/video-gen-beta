...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            safe_desc = asset_description.replace(' ', '_').upper()
            if asset_type == "music":
                asset_filename = f"{safe_desc}.mp3"
            elif asset_type == "voiceover":
                 asset_filename = f"{safe_desc}.mp3"
            else:
                asset_filename = f"{safe_desc}.mp4"
        ...
        if asset_type == "music":
            ...
            # Default to video procedural fallback
            return {
                "source": "procedural",
                "type": "solid_color",
...
def renderer_stub(blueprint):
    ...
    if bg_music:
        logs.append(f"Global Audio Track: {bg_music.get('desc', 'Unknown')} ({bg_music.get('type', 'Unknown')})")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Log Video
        logs.append(f"  > [VIDEO] Requesting Asset: '{asset.get('path', asset.get('desc', 'PROCEDURAL'))}'")
        logs.append(f"  > [VIDEO] Layer Used: {asset['layer_used']}")
        ...
        # Log Audio (VO)
        if vo_asset:
             # Use subtype for description if procedural, or use desc/path
            desc = vo_asset.get('desc', vo_asset.get('path', vo_asset.get('type', 'Unknown')))
            logs.append(f"  > [AUDIO] Voiceover Asset: '{desc}'")
            logs.append(f"  > [AUDIO] Layer Used: {vo_asset['layer_used']}")
