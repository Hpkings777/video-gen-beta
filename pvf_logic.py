...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            ext_map = {"video": ".mp4", "music": ".mp3", "voiceover": ".mp3"}
            extension = ext_map.get(asset_type, ".dat")
        ...
        if asset_type == "video":
            ...
            return {
                "source": "procedural",
                "type": "sine_wave",
                "description": "Tension Soundscape (Sine Wave Drone)",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
            ...
            return {
                "source": "procedural",
                "type": "tts_robot",
                "description": "Robotic Speech (System Default)",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Audio Track
        global_audio_desc = f"Background_Track_{style_preset_name}"
        global_audio = self.asset_manager.get_asset(global_audio_desc, failure_mode, asset_type="music")
        ...
        for idx, text in enumerate(segments):
            ...
            visual_asset_data = self.asset_manager.get_asset(asset_desc, failure_mode, asset_type="video")

            # Apply Style specific procedural overrides if needed
            if visual_asset_data["source"] == "procedural":
                if style_preset_name == "Dopamine_Spike":
                    visual_asset_data["color"] = "#FF00FF" # Neon Pink for fallback in dopamine
                else:
                    visual_asset_data["color"] = "#222222" # Dark Grey for fallback in stoic

            # Get Voiceover Asset
            voiceover_data = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")

            segment_data = {
                "id": f"seg_{idx:03d}",
                "text_content": text,
                "duration": round(duration, 2),
                "visual_asset": visual_asset_data,
                "audio_asset": voiceover_data,
                "style_config": {
                    "font": style["font"],
...
def renderer_stub(blueprint):
    ...
    # Log Global Audio
    g_audio = blueprint.get("global_audio", {})
    logs.append(f"Global Audio Track: {g_audio.get('description', 'Unknown')}")
    logs.append(f"  > Source: {g_audio.get('source', 'Unknown')}")
    logs.append(f"  > Layer Used: {g_audio.get('layer_used', 'Unknown')}")
    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        visual = segment["visual_asset"]
        voice = segment.get("audio_asset", {})
        ...
        if visual["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {visual['type']} (Color: {visual['color']})"
        logs.append(f"  > [Visual] Action: {action}")

        # Audio Log
        logs.append(f"  > [Audio] Voiceover Layer Used: {voice.get('layer_used', 'Unknown')}")
        if voice.get("source") == "procedural":
             logs.append(f"  > [Audio] Generating: {voice.get('description', 'TTS')}")
