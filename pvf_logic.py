    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "music":
             return {
                ...
                "details": "150Hz Sine Wave Drone (Code Generated)",
            ...
            return {
                ...
                "type": "tts_basic",
                "desc": "Robotic Speech",
                "details": "Local System TTS / Text-Only Mode",
                "layer_used": "Layer 3 (Procedural Backup)"
            }

        else: # Default to video/visual
            return {
                "source": "procedural",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Uses style preset name as the key for the music track
        ...
        for idx, text in enumerate(segments):
            ...
            # Apply Style specific procedural overrides if needed (only for visual generators)
            if visual_asset["source"] == "procedural" and visual_asset.get("type") == "video_generator":
            ...
            # Get Voiceover Asset
            # Use the text content itself as the description source
            vo_desc = text[:20]
...
def renderer_stub(blueprint):
    ...
    if audio:
        desc = audio.get("path") if audio["source"] == "local" else audio.get("desc")
    ...
    for segment in blueprint["timeline"]:
        ...
        if vo:
            vo_desc = vo.get("path") if vo["source"] == "local" else vo.get("desc")
        ...
        if asset["source"] == "local":
            ...
            desc = asset.get('desc', asset['type'])
