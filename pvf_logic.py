...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        # Default Layer 3 response (Video)
        layer3_asset = {
            "source": "procedural",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        global_audio = self.asset_manager.get_asset(
            f"Music_Track_{style_preset_name}",
            failure_mode,
            asset_type="music"
        )
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            # Using text content (first 20 chars) as description
            voiceover_asset = self.asset_manager.get_asset(
                f"VO_{text[:20]}",
                failure_mode,
...
def renderer_stub(blueprint):
    ...
    if audio:
        logs.append(f"Global Audio Track: {audio.get('style', audio.get('type', 'Unknown'))}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > [VIDEO] Requesting Asset: '{asset.get('path', asset.get('style', 'PROCEDURAL'))}'")
        ...
        if asset["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {style_info} ({asset.get('details', '')}) (Color: {asset.get('color', 'N/A')})"
        ...
        # Voiceover Log
        if vo:
            logs.append(f"  > [AUDIO] Voiceover: {vo.get('style', vo.get('type', 'Unknown'))}")
