...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            filename_prefix = ""
            if asset_type == "music":
                filename_prefix = "Music_Track_"
            elif asset_type == "voiceover":
                filename_prefix = "VO_"

            asset_filename = f"{filename_prefix}{asset_description.replace(' ', '_')}"

            # Preserve case for audio, upper for video (based on existing logic)
            if asset_type == "video":
                asset_filename = asset_filename.upper() + ".mp4"
            else:
                 asset_filename = asset_filename + ".mp3"
        ...
        if asset_type == "video":
            return {
             ...
             return {
                ...
                "details": "Code-generated, low-frequency Sine Wave Drone (60Hz)",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "Robotic Speech",
                "details": "Local TTS Engine (espeak) / Subtitled Text Only",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        ...
        return {
             "source": "procedural",
             "type": "unknown",
             "layer_used": "Layer 3 (Generic Backup)"
        }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            # Use first 20 chars of text for description
            vo_desc = text[:20]
...
def renderer_stub(blueprint):
    ...
    if audio.get("source") == "procedural":
        logs.append(f"  > Details: {audio.get('details', 'No details')}")
    else:
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > Requesting Video: '{video_asset.get('path', 'PROCEDURAL')}'")
        logs.append(f"  > Layer Used: {video_asset['layer_used']}")

        if video_asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local File: {video_asset['path']}"
        else:
            action = f"Rendering {segment['duration']}s of Procedural {video_asset['type']} (Color: {video_asset.get('color', '#000000')})"
        logs.append(f"  > Action: {action}")

        # Audio Log (Voiceover)
        logs.append(f"  > [AUDIO] Voiceover: {audio_asset.get('type', 'Unknown')}")
        if audio_asset.get("source") == "procedural":
             logs.append(f"  > Details: {audio_asset.get('details')}")
