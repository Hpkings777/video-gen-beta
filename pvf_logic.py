...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        # MUSIC FALLBACK: Sine Wave Drone
        elif asset_type == "music":
             return {
                "source": "procedural",
                "type": "Sine Wave Drone",
                "frequency": "Low (40Hz)",
                "description": "Tension Soundscape",
                "layer_used": "Layer 3 (Procedural Backup)"
            }

        # VOICEOVER FALLBACK: Robotic Speech
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "Robotic Speech",
                "engine": "Local TTS / Subtitles",
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Global Music Track (One for the whole video)
        # Using style preset name as the "mood" for music request
        music_asset = self.asset_manager.get_asset(f"{style_preset_name} Music", failure_mode, asset_type="music")
        ...
        for idx, text in enumerate(segments):
            ...
            if video_asset["source"] == "procedural" and video_asset["type"] == "CRT_INTERFACE":
...
def renderer_stub(blueprint):
    ...
    if music:
         logs.append(f"Global Audio Track: {music.get('type', 'Unknown')}")
         logs.append(f"  > Source: {music.get('source', 'Unknown')} | Layer: {music.get('layer_used', 'Unknown')}")
         if music.get('source') == 'procedural':
             logs.append(f"  > Description: {music.get('description', '')} ({music.get('frequency', '')})")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Video Log
        logs.append(f"  > [VIDEO] Requesting: '{v_asset.get('path', 'PROCEDURAL')}'")
        ...
        if a_asset.get('source') == 'procedural':
             logs.append(f"  > Engine: {a_asset.get('engine', 'Unknown')}")
