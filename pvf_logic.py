import os
import logging

# Configure Logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger("PVF_Logic")

class AssetManager:
    """
    The Resilience Layer.
    Manages asset fetching with a 3-layer fallback mechanism.
    """
    def __init__(self, asset_dir="assets/"):
        self.asset_dir = asset_dir
        if not os.path.exists(self.asset_dir):
            os.makedirs(self.asset_dir)

    def get_asset(self, asset_description, failure_mode=0, asset_type="video"):
        """
        Fetch an asset based on description and failure mode.
        Layers:
        1. External API (Simulated)
        2. Local Cache (File Check)
        3. Procedural Generation (Never Nothing)

        failure_mode:
          0 = Normal (Try L1 -> L2 -> L3)
          1 = Simulate L2 Failure (Force L3)
          2 = Bhuchal Mode (Force L3 immediately)

        asset_type: "video", "music", "voiceover"
        """

        # --- LAYER 1: EXTERNAL API (Simulated) ---
        # In a real app, this would call an API.
        # Here, we assume L1 always "fails" or times out for this demo,
        # pushing us to L2, unless we implement a mock L1 later.
        # For this logic, let's assume L1 fails to show the local/procedural resilience.
        # logger.info("Layer 1 (API) check skipped/failed.")

        # --- LAYER 2: LOCAL ASSETS ---
        # Normalize description to filename
        safe_desc = "".join(x for x in asset_description if x.isalnum() or x in [' ', '_']).replace(' ', '_')

        # Determine filename based on type
        if asset_type == "video":
            asset_filename = f"{safe_desc.upper()}.mp4"
        elif asset_type == "music":
            asset_filename = f"Music_Track_{safe_desc}.mp3"
        elif asset_type == "voiceover":
            asset_filename = f"VO_{safe_desc}.mp3"
        else:
            asset_filename = f"{safe_desc}.dat"

        if failure_mode == 0 and os.path.exists(os.path.join(self.asset_dir, asset_filename)):
            # Simple handling to change extension based on type if we were implementing real logic
            # But for simulation mode 0, we just return a success packet.

            return {
                "source": "local",
                "path": os.path.join(self.asset_dir, asset_filename),
                "type": asset_type,
                "layer_used": "Layer 2 (Local Asset)",
                "desc": asset_description
            }

        elif failure_mode == 1:
            # Simulate Layer 2 Failure (File not found)
            logger.warning("Layer 2 Failed: File not found (Simulated or Real)")
            pass # Continue to Layer 3

        elif failure_mode == 2:
            # Bhuchal Mode - Force skip to Layer 3
            pass

        # --- LAYER 3: PROCEDURAL BACKUP ---
        # Generates simple data structure based on asset_type

        if asset_type == "video":
            return {
                "source": "procedural",
                "type": "backup_generator::CRT_INTERFACE",
                "details": "Animated binary text overlay",
                "color": "#000000", # Default black
                "text": "SYSTEM FALLBACK: " + asset_description,
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "music":
             return {
                "source": "procedural",
                "type": "backup_generator::SINE_WAVE",
                "details": "Low-freq 60Hz Sine Wave",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "TTS_ROBOTIC",
                "details": "Subtitled Text Only",
                "text": asset_description, # The text to be spoken
                "layer_used": "Layer 3 (Procedural Backup)"
            }

        # Fallback for unknown types
        return {
            "source": "procedural",
            "type": "generic_fallback",
            "details": "Unknown Asset Type",
            "layer_used": "Layer 3 (Procedural Backup)"
        }

class JSONBuilder:
    """
    The Intent Layer.
    Translates script and style into the final JSON Blueprint.
    """

    STYLE_PRESETS = {
        "Dopamine_Spike": {
            "min_duration": 1.5,
            "max_duration": 3.0,
            "font": "Montserrat-Black",
            "font_size": 80,
            "primary_color": "#00FF00", # Neon Green
            "background_color": "#111111",
            "effects": ["shake", "flash", "glitch"],
            "transition": "cut_fast"
        },
        "Cinematic_Stoic": {
            "min_duration": 4.0,
            "max_duration": 8.0,
            "font": "Garamond",
            "font_size": 50,
            "primary_color": "#E0E0E0", # Off-white
            "background_color": "#050505", # Deep black
            "effects": ["slow_zoom", "film_grain"],
            "transition": "fade_cross"
        }
    }

    def __init__(self):
        self.asset_manager = AssetManager()

    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        """
        Generates the JSON blueprint.
        """
        style = self.STYLE_PRESETS.get(style_preset_name, self.STYLE_PRESETS["Dopamine_Spike"])

        # Global Audio Track (Music)
        # Using style preset name as the "description" or "vibe" for the music
        global_audio = self.asset_manager.get_asset(style_preset_name, failure_mode, asset_type="music")

        # Parse script into segments (simple sentence/line split)
        segments = [s.strip() for s in script_text.split('.') if s.strip()]
        if not segments:
            segments = [script_text]

        timeline = []

        for idx, text in enumerate(segments):
            # Determine duration based on style
            # Simple logic: shorter text -> shorter duration, but clamped by style limits
            base_duration = len(text.split()) * 0.5
            duration = max(style["min_duration"], min(base_duration, style["max_duration"]))

            # Get Visual Asset
            # We use a keyword from the text as the asset description or ID
            asset_desc = " ".join(text.split()[:3])
            visual_asset = self.asset_manager.get_asset(asset_desc, failure_mode, asset_type="video")

            # Apply Style specific procedural overrides if needed for visual assets
            if visual_asset["source"] == "procedural" and visual_asset["type"] == "backup_generator::CRT_INTERFACE":
                if style_preset_name == "Dopamine_Spike":
                    visual_asset["color"] = "#FF00FF" # Neon Pink for fallback in dopamine
                else:
                    visual_asset["color"] = "#222222" # Dark Grey for fallback in stoic

            # Get Voiceover Asset
            # Use the text content as the description/source
            voiceover_asset = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")

            segment_data = {
                "id": f"seg_{idx:03d}",
                "text_content": text,
                "duration": round(duration, 2),
                "visual_asset": visual_asset,
                "voiceover": voiceover_asset,
                "style_config": {
                    "font": style["font"],
                    "font_size": style["font_size"],
                    "text_color": style["primary_color"],
                    "effect": style["effects"][idx % len(style["effects"])], # Cycle effects
                    "transition": style["transition"]
                }
            }
            timeline.append(segment_data)

        blueprint = {
            "project_name": "PVF_Demo_Project",
            "style_preset": style_preset_name,
            "total_duration": sum(s["duration"] for s in timeline),
            "global_audio": global_audio,
            "timeline": timeline
        }

        return blueprint

def renderer_stub(blueprint):
    """
    Iterates through the JSON timeline and output a step-by-step log.
    DO NOT execute ffmpeg.
    """
    logs = []
    logs.append(f"Starting Render Simulation for: {blueprint['project_name']}")
    logs.append(f"Style Preset: {blueprint['style_preset']}")

    # Log Global Audio
    audio = blueprint.get("global_audio", {})
    logs.append(f"Global Audio Track: {audio.get('type', 'Unknown')}")
    logs.append(f"  > Details: {audio.get('details', 'None')}")
    logs.append(f"  > Layer Used: {audio.get('layer_used', 'Unknown')}")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        v_asset = segment["visual_asset"]
        vo_asset = segment["voiceover"]

        logs.append(f"[Segment {segment['id']}] Duration: {segment['duration']}s")

        # Log Visuals
        logs.append(f"  > [VISUAL] Requesting Asset: '{v_asset.get('path', 'PROCEDURAL')}'")
        logs.append(f"  > Layer Used: {v_asset['layer_used']}")
        if v_asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local File: {v_asset['path']}"
        else:
            action = f"Rendering {segment['duration']}s of Procedural {v_asset['type']} ({v_asset.get('details', '')}) Color: {v_asset.get('color', 'N/A')}"
        logs.append(f"  > Action: {action}")
        logs.append(f"  > Applied Effect: {segment['style_config']['effect']}")

        # Log Voiceover
        logs.append(f"  > [AUDIO] Voiceover: {vo_asset.get('type', 'Unknown')}")
        logs.append(f"  > Details: {vo_asset.get('details', 'None')}")

        logs.append("-" * 20)

    logs.append("Render Simulation Complete.")
    return "\n".join(logs)
