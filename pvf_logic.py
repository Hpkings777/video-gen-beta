...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            # Construct mock filename based on type
            ext = "mp4"
            if asset_type == "music": ext = "mp3"
            elif asset_type == "voiceover": ext = "wav"
        ...
        # Resilience Matrix Implementation
        ...
        if asset_type == "video":
            # Video Clips -> Procedural Glitch Screen: backup_generator::CRT_INTERFACE with animated binary text overlay.
            fallback_asset.update({
                "type": "CRT_INTERFACE",
                "bg_color": "#000000",
                "overlay_text": "BINARY_OVERLAY",
                "description": f"GLITCH_VISUAL::{asset_description}"
            })

        elif asset_type == "music":
            # Music Track -> Tension Soundscape: Code-generated, low-frequency Sine Wave Drone.
            fallback_asset.update({
                "type": "sine_wave_drone",
                "frequency": "low_freq",
                "description": "TENSION_SOUNDSCAPE_DRONE"
            })

        elif asset_type == "voiceover":
            # Voiceover -> Robotic Speech: Local, self-hosted basic TTS or Subtitled Text Only.
            fallback_asset.update({
                "type": "robotic_speech",
                "engine": "local_fallback_tts",
                "description": f"ROBOTIC_READ::{asset_description}"
            })

        else:
             # Fallback for unknown types (Overlay/Text)
             fallback_asset.update({
                "type": "system_status_log",
                "text": "[PROTOCOL OVERRIDE] [L3 ACTIVE] [DATA_MISSING]",
                "description": "SYSTEM_STATUS_LOG"
             })
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Get Global Music for the project
        music_desc = "Background_Beat"
        global_music = self.asset_manager.get_asset(music_desc, failure_mode, asset_type="music")
        ...
        for idx, text in enumerate(segments):
            ...
            if visual_asset["source"] == "procedural" and visual_asset.get("type") == "CRT_INTERFACE":
                if style_preset_name == "Dopamine_Spike":
                    visual_asset["bg_color"] = "#FF00FF" # Neon Pink for fallback in dopamine
                else:
                    visual_asset["bg_color"] = "#222222" # Dark Grey for fallback in stoic
...
def renderer_stub(blueprint):
    ...
    # Global Audio Log
    music = blueprint.get("global_audio")
    if music:
        logs.append(f"Global Audio Track: {music.get('path', music.get('description', 'Unknown'))}")
        logs.append(f"  > Music Layer Used: {music['layer_used']}")
        if music['source'] == 'procedural':
            logs.append(f"  > Action: Generating {music['type']} ({music.get('frequency', 'N/A')})")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        asset = segment["visual_asset"]
        voiceover = segment.get("audio_asset")

        logs.append(f"[Segment {segment['id']}] Duration: {segment['duration']}s")

        # Visual Log
        logs.append(f"  > Visual Asset: '{asset.get('path', asset.get('description', 'PROCEDURAL'))}'")
        logs.append(f"  > Layer Used: {asset['layer_used']}")

        if asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local File: {asset['path']}"
        else:
            action = f"Rendering {segment['duration']}s of Procedural {asset['type']} (Color: {asset.get('bg_color', 'N/A')})"
        logs.append(f"  > Action: {action}")

        # Voiceover Log
        if voiceover:
             logs.append(f"  > Voiceover: {voiceover.get('path', voiceover.get('description', 'N/A'))}")
             logs.append(f"  > VO Layer Used: {voiceover['layer_used']}")
             if voiceover['source'] == 'procedural':
                 logs.append(f"  > VO Action: Generating {voiceover['type']} ({voiceover.get('engine', 'N/A')})")
