    ...
    def get_asset(self, asset_description, failure_mode, asset_type='video'):
        ...
        if failure_mode == 0:
            ...
            return {
                ...
                "type": asset_type,
                "desc": asset_description, # Pass original description for logging
        ...
        layer_name = "Layer 3 (Procedural Backup)"

        if asset_type == 'music':
             return {
                "source": "procedural",
                "type": "sine_wave",
                "freq": 60, # Low freq drone
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Request Global Audio Track (Music)
        ...
        for idx, text in enumerate(segments):
            ...
            audio_asset = self.asset_manager.get_asset(text, failure_mode, asset_type='voiceover')
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        ...
        logs.append(f"  > Source: {global_audio.get('source', 'unknown')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Audio Log
        if audio:
             logs.append(f"  > Voiceover: {audio.get('desc', 'Unknown')}")
