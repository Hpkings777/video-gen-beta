...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        # Voiceover: Robotic Speech
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "robotic_speech",
                "text": asset_description, # The text to speak
                "description": "Robotic Speech: Local, self-hosted basic TTS",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        # Determine Global Audio (Music)
        # Using a fixed description for the demo
        music_desc = "background_music"
        ...
        for idx, text in enumerate(segments):
            ...
            if visual_asset_data["source"] == "procedural" and visual_asset_data["type"] == "CRT_INTERFACE":
            ...
            # Get Voiceover Asset
            voiceover_asset_data = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")
            ...
            segment_data = {
                ...
                "voiceover": voiceover_asset_data,
                "style_config": {
                    "font": style["font"],
...
def renderer_stub(blueprint):
    ...
    if global_audio:
        logs.append(f"Global Audio Track: {global_audio.get('type')}")
        logs.append(f"  > Source: {global_audio['source']}")
        logs.append(f"  > Layer: {global_audio['layer_used']}")
        if global_audio['source'] == "procedural":
             logs.append(f"  > Description: {global_audio.get('description')}")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        visual_asset = segment["visual_asset"]
        voiceover_asset = segment["voiceover"]
        ...
        # Voiceover
        logs.append(f"  > Voiceover Asset: {voiceover_asset.get('type')}")
        logs.append(f"  > Layer Used: {voiceover_asset['layer_used']}")
        if voiceover_asset['source'] == "procedural":
             logs.append(f"  > Voice Action: Generating {voiceover_asset['type']} for text '{segment['text_content'][:20]}...'")
