    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if asset_type == "video":
             return {
                ...
                "color": "#000000", # Default black, will be overridden by JSONBuilder if needed
             ...
             return {
                ...
                "details": "Tension Soundscape (Low Frequency)",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "voiceover":
             return {
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover Asset
            vo_desc = text[:20] # Use text as ID/desc
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            action = f"Rendering {segment['duration']}s of Local File: {asset['path']}"
             ...
             style_info = asset.get('style', asset.get('type'))
             action = f"Rendering {segment['duration']}s of Procedural {style_info} ({asset.get('details', '')}) (Color: {asset.get('color')})"
        else:
            action = f"Rendering {segment['duration']}s of Unknown Asset"
