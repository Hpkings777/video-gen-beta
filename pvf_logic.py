...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            # Prefix for clarity (V_, M_, S_)
            prefix_map = {"video": "", "music": "Music_Track_", "voiceover": "VO_"}
        ...
        # Generates data structure based on asset_type
        ...
        if asset_type == "video":
            return {
             ...
             return {
                ...
                "freq": "60Hz",
                "details": "Tension Soundscape Drone",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
        elif asset_type == "voiceover":
             return {
                "source": "procedural",
                "type": "backup_generator::ROBOTIC_SPEECH",
                "engine": "espeak",
                "details": "Robotic TTS Fallback",
                "layer_used": "Layer 3 (Procedural Backup)"
            }
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Voiceover
            voiceover_data = self.asset_manager.get_asset(text[:20], failure_mode, asset_type="voiceover")

            # Apply Style specific procedural overrides if needed
            # Only for visual assets
            if asset_data["source"] == "procedural" and "backup_generator::CRT_INTERFACE" in asset_data["type"]:
...
def renderer_stub(blueprint):
    ...
    if "global_audio" in blueprint:
        ga = blueprint["global_audio"]
        logs.append(f"Global Audio Track: {ga.get('desc', 'Unknown')}")
        logs.append(f"  > Layer Used: {ga['layer_used']}")
        if ga["source"] == "local":
             logs.append(f"  > Source: {ga['path']}")
        else:
             logs.append(f"  > Details: {ga.get('details', 'Procedural Audio')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        if asset["source"] == "local":
            ...
            # Handle procedural color override safely
            color = asset.get('color', 'N/A')
            action = f"Rendering {segment['duration']}s of Procedural {asset.get('desc', asset['type'])} (Color: {color})"

        logs.append(f"  > Action: {action}")

        # Audio Log (Voiceover)
        if voiceover:
             logs.append(f"  > [AUDIO] Voiceover: {voiceover.get('desc', 'Unknown')}")
             logs.append(f"  > Layer Used: {voiceover['layer_used']}")
             if voiceover["source"] == "local":
                logs.append(f"  > Source: {voiceover['path']}")
             else:
                logs.append(f"  > Details: {voiceover.get('details', 'Procedural Voiceover')}")
