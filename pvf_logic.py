...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        for idx, text in enumerate(segments):
            ...
            # Get Asset (Voiceover)
            vo_data = self.asset_manager.get_asset(text, failure_mode, asset_type="voiceover")
...
def renderer_stub(blueprint):
    ...
    if g_audio:
        logs.append(f"Global Audio Track: {g_audio.get('desc', 'Music Track')}")
        logs.append(f"  > Source: {g_audio.get('path', 'PROCEDURAL')}")
    ...
    for segment in blueprint["timeline"]:
        ...
        # Log Voiceover
        if vo_asset:
             logs.append(f"  > Voiceover: {vo_asset.get('desc', 'Voiceover Track')}")
