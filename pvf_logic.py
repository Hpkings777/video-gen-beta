...
class AssetManager:
    ...
    def get_asset(self, asset_description, failure_mode, asset_type="video"):
        ...
        if failure_mode == 0:
            ...
            # Simple extension mapping for simulation
            ext_map = {"video": "mp4", "music": "mp3", "voiceover": "wav"}
        ...
        if asset_type == "video":
            ...
            fallback_asset.update({
                ...
                "description": "Tension Soundscape: Low-frequency Sine Wave"
            })
        elif asset_type == "voiceover":
            fallback_asset.update({
                "type": "robotic_speech",
                "description": "Local TTS / Subtitled Text Only"
            })
        else:
            fallback_asset.update({
                "type": "generic_fallback",
                "description": "Unknown asset type fallback"
            })
...
class JSONBuilder:
    ...
    def build_blueprint(self, script_text, style_preset_name, failure_mode):
        ...
        music_desc = f"{style_preset_name}_Background_Music"
...
def renderer_stub(blueprint):
    ...
    logs.append(f"Global Audio Track: {global_audio.get('type', 'Unknown')}")
    logs.append(f"  > Layer Used: {global_audio.get('layer_used', 'N/A')}")
    if global_audio.get("source") == "procedural":
        logs.append(f"  > Description: {global_audio.get('description', 'N/A')}")

    logs.append("-" * 40)

    for segment in blueprint["timeline"]:
        visual = segment["visual_asset"]
        voiceover = segment.get("voiceover_asset", {})

        logs.append(f"[Segment {segment['id']}] Duration: {segment['duration']}s")

        # Visual Log
        logs.append(f"  [VISUAL]")
        logs.append(f"  > Requesting Asset: '{visual.get('path', 'PROCEDURAL')}'")
        ...
        if visual["source"] == "local":
            ...
            action = f"Rendering {segment['duration']}s of Procedural {visual['type']} (Color: {visual.get('color', 'N/A')})"
        logs.append(f"  > Action: {action}")

        # Voiceover Log
        logs.append(f"  [VOICEOVER]")
        logs.append(f"  > Type: {voiceover.get('type', 'Unknown')}")
        logs.append(f"  > Layer Used: {voiceover.get('layer_used', 'N/A')}")
        if voiceover.get("source") == "procedural":
             logs.append(f"  > Description: {voiceover.get('description', 'N/A')}")
